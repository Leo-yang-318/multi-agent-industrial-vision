from utils.result_schema import build_result
...
def get_part_mask(gray):  # 获取主体工件避免误检背景
    blur = cv2.GaussianBlur(gray, (5, 5), 0)
    _, binary = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)  # Otsu自动阈值分割
...
def get_filled_part_mask(part_mask):  # 孔洞排除
    """
    生成填满孔洞的工件 mask。
...
def detect_holes_by_contour(gray, part_mask):
    """
    检测工件中的孔洞数量。
    返回检测到的孔列表。
    """
...
def detect_missing_hole(holes, expected_hole_count):
    """
    根据孔数量判断是否漏孔。
    """
    detected_hole_count = len(holes)

    result = {
        "type": "missing_hole",
        "name": "漏孔",
        "expected_hole_count": int(expected_hole_count),
        "detected_hole_count": int(detected_hole_count),
        "has_missing_hole": detected_hole_count < expected_hole_count
    }

    if detected_hole_count < expected_hole_count:
        result["missing_count"] = expected_hole_count - detected_hole_count
    else:
...
def grow_scratch_bbox(seed_contour, response, part_mask):  # 扩展划痕
    """
    沿着种子划痕方向扩展检测框。
...
def is_same_scratch(candidate, existing):  # 重复缺陷合并
...
def run_hole_detection(
        image_path: str,
        output_dir="outputs/traditional_defect",
        expected_hole_count=7
):
    ...
    holes = detect_holes_by_contour(
        gray,
        part_mask
    )

    result = detect_missing_hole(
        holes,
        expected_hole_count
    )
...
def run_scratch_detection(
    ...
    holes = detect_holes_by_contour(gray, part_mask)

    missing_hole_result = None
    if expected_hole_count is not None:
        missing_hole_result = detect_missing_hole(
            holes,
            expected_hole_count
        )
    ...
    has_missing_hole = (
            missing_hole_result is not None and
            missing_hole_result["has_missing_hole"]
    )

    result = {
        "image_path": str(image_path),
        "task": "scratch_and_missing_hole_detection",
        "has_defect": len(scratch_defects) > 0 or has_missing_hole,
        "scratch_defect_count": len(scratch_defects),
        "detected_hole_count": len(holes),
        "holes": holes,
        "missing_hole_result": missing_hole_result,
...
class TraditionalDefectSkill:
    name = "traditional_defect_skill"
    ...
    def run(
        ...
        # ------------------------
            ...
            result = run_hole_detection(
                ...
                expected_hole_count=params.get(
